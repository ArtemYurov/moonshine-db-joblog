...
`ArtemYurov\JobLog\Middleware\JobLogWithoutOverlapping` сериализует Loggable job по их тегам
JobLog: когда воркер берёт job, чьи теги уже находятся в статусе `PROCESSING` у соседнего запуска,
текущий запуск ждёт или отбрасывается — в обоих случаях завершаясь каноническими для Laravel
`PROCESSED`/`FAILED` (без кастомного статуса), опираясь на таблицу JobLog (без cache-лока). Пакет
не подключает middleware сам — оркестратор
([`moonshine-command-schedule-job`](https://github.com/ArtemYurov/moonshine-command-schedule-job))
включает его для job в момент диспатча; теги берутся из собственной записи JobLog этой job.

#### Сериализация или отбрасывание

Режим задаётся через задержку релиза, повторяя нативный
...
- **сериализация** (`releaseAfter` задан): job возвращается в очередь и повторяется, пока теги не
  освободятся → `PROCESSED` (в итоге выполнилась) или `FAILED` (закончились `tries`/`retryUntil`).
- **отбрасывание** (`dontRelease()`, `releaseAfter = null`): возврат без выполнения — соседний
  запуск уже делает эту работу, поэтому текущий избыточен → `PROCESSED` через обычный успешный
  жизненный цикл Laravel. На sync-очереди `release()` ничего не делает, поэтому сериализованный
  запуск там тоже просто завершается как `PROCESSED`.
...
> попытку и она завершится с `MaxAttemptsExceeded`. Именно это ограничивает сериализованную job
> статусом `FAILED` вместо бесконечных релизов.

Проверка выполняется `SELECT`-запросом с тай-брейком по `queued_at`+`uuid`; полностью конкурентная
гонка — известное (редкое) окно TOCTOU.
